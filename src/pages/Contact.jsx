...
const Contact = () => {
  ...
  return (
    ...
    <div className="container flex-col contact-page">
      <h2>Contacts</h2>
      <h3 onClick={()=>setIsMedInfoShown(prev=>!prev)} className='flex-row pointer accordion-icon'>Medicines Information Contacts<FontAwesomeIcon  icon={isMedInfoShown ? faChevronUp : faChevronDown}  />
</h3>
{isMedInfoShown&&renderContacts(medicinesInfo)}
<h3 onClick={()=>setIsAnticoagShown(prev=>!prev)} className='flex-row pointer accordion-icon'>Anticoagulation Clinic Contacts<FontAwesomeIcon  icon={isAnticoagShown ? faChevronUp : faChevronDown}  />
</h3>
